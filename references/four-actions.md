...
- Response 3 uses structured format in this order: `**Verdict:**` → `**Confidence:**` → `[→ To reach [next level]:]` (conditional, Medium/Low only) → `**Stop if** *(conditions that would change this bet):*` → `**Blind spots:**` (conditional) → `**Truth fingerprint:**`
